...
import static com.nanowheel.nanoux.nanowheel.util.Util.rpmToKilometersPerHour;
...
public class OWDevice extends BaseObservable implements DeviceInterface {
    ...
    public final ObservableField<Integer> onewheelType = new ObservableField<>(); //0 for OG, 1 for + & XR, 2 for Pint
...
*/
    ...
    boolean updateBatteryChanges = true;
    ...
    public void setupCharacteristics(Context context) {
        ...
        //Read once a minute (Thanks biell)
    ...
    public void processUUID(BluetoothGattCharacteristic incomingCharacteristic) {
        ...
        if (dc == null){
            Log.w("ONEWAVE","RECIEVED UNKNOWN CHARACTERISTIC " + incomingUuid + " " + Util.unsignedShort(incomingValue));
            return;
        }
        ...
        if(dev_uuid != null && dev_uuid.equals(incomingUuid)) {
            switch(dev_uuid) {
                ...
                case OnewheelCharacteristicFirmwareRevision:
                    processFirmwareRevision(incomingValue, dc);
                    break;
    ...
    private void processHardwareRevision(byte[] incomingValue, DeviceCharacteristic dc) {
        int revision = Util.unsignedShort(incomingValue);
        dc.value.set(Integer.toString(revision));
        battery.setHardware(revision);
    }

    private void processFirmwareRevision(byte[] incomingValue, DeviceCharacteristic dc) {
        int revision = Util.unsignedShort(incomingValue);
        if (revision < 4020){//Version of andromeda, only came out for +. Hoping this is a good cut off
            onewheelType.set(0);
        }else if (revision >= 4020 && revision < 5000){
            onewheelType.set(1);
        }else{
            onewheelType.set(2);
        }
    ...
    private void processOdometer(byte[] incomingValue, DeviceCharacteristic dc) {
        ...
        if (dc_odometer != null) {
            if (SharedPreferencesUtil.getPrefs(context).isMetric()) {
                ...
                dc_odometer.value.set(Util.revolutionsToKilometers((double) i_odometer,onewheelType.get()) + "");
            } else {
                //dc_odometer.value.set(String.format(Locale.getDefault(),"%3.2f", revolutionsToMiles((double) i_odometer)));
                dc_odometer.value.set(Util.revolutionsToMiles((double) i_odometer,onewheelType.get()) + "");
            }
        }
    ...
    private long lastSpeedStamp = -1;
    private boolean recordValid = true;
    private void processSpeedRpm(byte[] incomingValue, DeviceCharacteristic dc) {
        ...
        if (rpmToKilometersPerHour(i_speedRpm,onewheelType.get()) <= 5){
            recordValid = true;
        }
        Date date = new Date();
        if (lastSpeed == -1){
            lastSpeed = i_speedRpm;
            lastSpeedStamp = date.getTime();
        }
        long timeDiff = (date.getTime() - lastSpeedStamp) / 1000;
        if (timeDiff != 0){
            float acceleration = Math.abs((i_speedRpm - lastSpeed) / timeDiff);
...
//        Timber.d("controllerTemp = " + controllerTemp);
    ...
    private void setFormattedSpeedWithMetricPreference(DeviceCharacteristic deviceCharacteristic, double speedRpm) {
        boolean isMetric = SharedPreferencesUtil.getPrefs(context).isMetric();
        deviceCharacteristic.value.set(String.format(Locale.getDefault(), "%3.2f", isMetric ? rpmToKilometersPerHour((double) speedRpm,onewheelType.get()) : Util.rpmToMilesPerHour((double) speedRpm,onewheelType.get())));
    }
    ...
    private void processCurrentAmps(byte[] incomingValue, DeviceCharacteristic dc) {
        ...
        if (onewheelType.get() == null || onewheelType.get() > 0) {
            multiplier = 1.8f;
        } else {
            multiplier = 0.9f;
        }
    ...
    private void updateChargeDist( float dist){
        ...
        if (lastDist < 0){
            if (characteristics.get(OnewheelCharacteristicBatteryRemaining).value.get() != null){
                ...
                if (SharedPreferencesUtil.getPrefs(context).isMetric()) {
                    characteristics.get(MockOnewheelCharacteristicOdometerCharge).value.set(Util.revolutionsToKilometers(SharedPreferencesUtil.getPrefs(context).getChargeDistRev(),onewheelType.get()) + "");
                }else{
                    characteristics.get(MockOnewheelCharacteristicOdometerCharge).value.set(Util.revolutionsToMiles(SharedPreferencesUtil.getPrefs(context).getChargeDistRev(),onewheelType.get()) + "");
                }
            }
        }else{
            float battery = Util.parseF(characteristics.get(OnewheelCharacteristicBatteryRemaining).value.get());
    ...
    private void calcRange(int battery){
        ...
        if (battStartCalc >= 0 && battery < battStartCalc && rStart >= 0) {
            if (characteristics.get(OnewheelCharacteristicOdometer).value.get() != null) {
                ...
                if (SharedPreferencesUtil.getPrefs(context).isMetric()) {
                    characteristics.get(MockOnewheelCharacteristicOdometerRange).value.set(Util.revolutionsToKilometers(revsRem,onewheelType.get()) + "");
                } else {
                    characteristics.get(MockOnewheelCharacteristicOdometerRange).value.set(Util.revolutionsToMiles(revsRem,onewheelType.get()) + "");
                }
            }
        }
    ...
    private static String[] modesListPint = {"Uh Oh","Uh Oh","Uh Oh","Uh Oh","Uh Oh","Redwood","Pacific","Elevated","Skyline","Uh Oh"};
    private static String[] modeSpeeds = {"Uh oh","12","15","15","12","15","19","19","20",""};
    private static String[] modeSpeedsPint = {"Uh oh","Uh oh","Uh oh","Uh oh","Uh oh","12","16","16","16","Uh oh"};
    ...
    public void modeChange(View view, int ID, int ID2){
        ...
        if(value == null)  return;
        final int disVal = Math.round(Util.parseF(value));
        final int owType = onewheelType.get();
        textMode.post(new Runnable() {
            ...
            public void run() {
                if (owType < 2) {
                    ...
                    textMode.setText(modesListPint[disVal]);
                }
            }
        });
        ...
        textSpeed.post(new Runnable() {
            ...
            public void run() {
                if (owType < 2) {
                    ...
                    textSpeed.setText(modeSpeedsPint[disVal]);
                }
            }
        });
